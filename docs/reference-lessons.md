...
采用先探索项目、在确有多个有效方向时先比较 2-3 个整体方案，再按架构/组件与接入/数据模型与数据流/错误处理/验证的章节走读。每一章先呈现连贯的目标设计；仍有取舍时才逐项提问，章节没有未决问题后再让用户确认或修正。组件需说明用途、消费者、拥有的数据或状态、依赖方向、稳定契约和可替换内部。Design 落盘后自检实际写入的文件，再由用户审阅本地设计记录，避免对话结论在文档中走样。定义 Design Ready gate，以及 Visual Companion 的 just-in-time 原则。方案数量不是仪式：仓库证据唯一确定方向时不制造伪选项。没有采用“所有微小改动都必须完整 Design”、机械 TDD 或自动提交 Design 文档。
