...
采用先探索项目、在确有多个有效方向时先比较 2-3 个整体方案，再按依赖关系讨论真正受影响的架构、接入、接口、数据、失败行为和验证。AWK 同样使用自然的一问一答和连贯设计章节，而不是固定决策卡；已经由事实与早先选择唯一确定的内容进入章节整体审阅，不再逐维度索要批准。架构、数据结构和数据流使用紧凑树与示意代码，且 AWK 额外要求每个代码变化都有对应目标语言示意代码。Superpowers 采用作者快速自检 written spec 后再交给用户审阅；AWK 改由最小上下文的独立只读 subagent 检查实际文件。最终审阅的直接接受同时授权 Act，worktree 选择仍作为执行阶段的新决定。保留 Design Ready gate 和 Visual Companion 的 just-in-time 原则，不采用机械 TDD 或自动提交 Design 文档。
...
采用其最有价值的 grounding 与闭环思想：首个实质问题前按真实生产路径、相似实现和适用参考建立依据；可查证的存在/缺失声明必须查源；成功标准用一般规则、必要场景和证据变得可验收；跨生产路径的结果逐路径证明；Review 完整覆盖实际影响并按风险调整调查深度；长期知识由下一次 Design 回读。没有照搬固定读取配额、临时 grounding dossier、独立 Plan、固定 persona 队列、内部编号体系、多 provider 路由或插件自建 `solutions/` 知识库。
