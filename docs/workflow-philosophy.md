...
Design 的边界由共同的用户结果、领域不变量、稳定契约和原子验收结果决定，不按目录、模块或调用路径机械拆分。同一个结果依赖的 Runtime、Editor、Bake 或其他生产路径即使彼此独立实现，也必须放进同一份 Design，避免不同路径形成不同语义。只有能够分别授权、验证、交付和回滚，且不共享实质设计决定或不变量的变更才拆开。
