...
证据也有作用域。当前代码、当前配置、某份资源样本、从特定输入推导的估算、外部平台合同和未来目标不能互相代替。精确数字只有在输入与计算可重建时才比定性结论更有价值；用户纠正一个前提后，所有依赖该前提的接口、数据格式、成本、理由和成功标准都必须一起修正，而不是只改被指出的句子。机械格式检查不能证明技术事实。
