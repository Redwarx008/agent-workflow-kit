...
实现速度会放大设计质量。边界、组件职责、数据结构与不变量、数据所有权、接口、实现机制、生产集成点和失败行为没有讲清楚时，快速编码只会更快地产生耦合与返工。Design 阶段的目标不是写得长，也不是把模板标题填满，而是通过调查和讨论把会改变实现方向的决定说清楚。
...
不要依赖单个对话无限增长。项目应有稳定的启动阅读顺序、活跃 Design、事实执行账本、会话结束记录和可复用 learnings。原始过程日志应定期压缩成决策或经验，而不是永久把全部历史塞进上下文。
