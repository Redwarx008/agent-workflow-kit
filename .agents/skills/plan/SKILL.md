---
name: plan
description: Converts a Ready workflow Design into a traceable, decision-complete implementation Plan and waits for explicit execution authorization. Use after design reaches Ready, when the user asks to plan an active change, or when an existing Plan must be revised before implementation.
---
...
Read the active `design.md`, [assets/plan.md](assets/plan.md), related repository code/docs, and any existing `plan.md`. If Design is missing, not Ready, contradictory, or still contains an implementation-shaping question, return to `$agent-workflow-kit:design`.
...
Summarize the Plan and ask for explicit implementation authorization. Set `Authorization: Approved` only after the user authorizes it. Do not invoke `$agent-workflow-kit:act` before that response.
