#!/usr/bin/env bash
# Stop the Agent Workflow Design Companion and clean up
# Usage: stop.sh <session_dir>
#
# Kills the server process. Only deletes session directory if it's
# under /tmp (ephemeral). Persistent directories (workflow/.local/) are
# kept so mockups can be reviewed later.

SESSION_DIR="$1"

if [[ -z "$SESSION_DIR" ]]; then
  echo '{"error": "Usage: stop.sh <session_dir>"}'
  exit 1
fi
