...
const COMPANION_NAME = 'Agent Workflow Design Companion';
...
let COOKIE_NAME = 'agent-workflow-key-' + PORT; // refined to the actual bound port in onListen
...
<html>
<head><meta charset="utf-8"><title>${COMPANION_NAME}</title>
<style>
...
<body><!-- BRANDING --><h1>${COMPANION_NAME}</h1>
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Opening ${COMPANION_NAME}</title></head>
<body>
<script>
try { sessionStorage.setItem('agent-workflow-session-key', ${jsonKey}); } catch (e) {}
location.replace('/');
</script>
</body>
</html>`;
...
function brandMarkup() {
  return '<div class="brand"><span class="brand-copy">' + COMPANION_NAME + '</span></div>';
}
...
// override with BRAINSTORM_IDLE_TIMEOUT_MS (start.sh: --idle-timeout-minutes).
...
function startServer() {
  ...
  function onListen() {
    ...
    COOKIE_NAME = 'agent-workflow-key-' + PORT;
