...
Follow the loaded process and contracts exactly; do not modify product files. Before each Design question, validate the exact user-facing message and then emit that content unchanged; never leave reasoning required to answer the question only in commentary. Complete the implementation or no-implementation Ready transition defined by the Design process.
