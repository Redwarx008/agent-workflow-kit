...
10. For every affected architecture, data-structure, or data-flow decision, separate current-state evidence from target proposals. Put only inspected current-state trees or code before the recommendation. Under each recommended or materially different alternative option, include that option's target tree or flow tree and illustrative code; an alternative that shares the recommendation's shape must say exactly what it shares and what differs. Record the selected target artifacts in the Design. Mark the transient evaluator transcript `architecture-decision`, `data-structure-decision`, or `data-flow-decision` so the evaluator checks these artifacts. Use the target language and label snippets illustrative; never replace a tree or code sketch with vague prose.
