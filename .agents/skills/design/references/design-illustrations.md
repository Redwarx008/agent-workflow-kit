...
Prefer a compact text tree and the target project's implementation language. Apply the dialogue contract for illustrative target code and its current-code display rule; trees complement rather than replace required code evidence. Omit a tree for a local change whose surrounding structure is unchanged, and omit all artifacts for unaffected dimensions.
