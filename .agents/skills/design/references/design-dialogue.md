...
Use headings only when they improve navigation in a long or structurally rich turn. Use domain titles that state what is being decided. Do not require fixed sections, A/B labels, repeated restatement of the same context, internal IDs, numbered cards, or a promised total such as `Design card 1/2` or `设计卡 1/2`; later evidence determines whether another question exists. Ask exactly one decision per message and wait for the answer before resolving a dependent decision. Selecting one, several, all, or none from an evidence-backed candidate set is one scope decision, not several confirmation gates.
