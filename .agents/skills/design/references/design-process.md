...
Maintain only `workflow/active/<change-name>/design.md` under the Design contract. Write the proved facts that frame the first decision after exploration, then update each selected result as soon as it is resolved. When real alternatives required a user question, do not write the recommended target before the explicit reply; after resolution, apply the Design contract's selection-source line to the final selected item. Rewrite superseded content instead of appending history. Evaluator and Visual Companion files are disposable `workflow/.local/` state.
...
4. Before editing Design, dispose every P0/P1 from the integration review as exactly one of:
   ...
   - **Mechanical:** cite the evidence that leaves only one contract-preserving correction and state its narrow boundary.
