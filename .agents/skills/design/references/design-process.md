...
2. If that reviewer identifies one or more exact questions it cannot reliably judge, launch the smallest sufficient number of suitable read-only subagents, each for only one question. The main reviewer retains the whole-Design verdict and integrates the returned evidence. A specialist `PASS` resolves only its question; `FAIL` or `BLOCKED` blocks Design.
