...
- Design 使用自然的一问一答，不强制五段卡片、A/B 标签或逐维度审批；只有真实未决选择才提问。凡提议涉及项目代码变化，都并排提供带仓库 `path:line` 的最小当前代码和对应目标语言示意代码；新增代码也要展示当前接入 seam，结构树不能代替代码。多个方案共享的当前或目标代码只展示一次，各方案只展示真正不同的部分。
